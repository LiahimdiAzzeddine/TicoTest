...
import { handleShareOptions } from "./Partage";
...
export default function ContributionPopupContent({ isOpen, setIsOpen, organization }) {
    ...
    const handleIncrement = () => setAmount((prev) => Math.min(prev + 1, remaining));
    ...
    const handleContribution = () => {
        ...
        if (amount > remaining) {
            toast.error(`Le montant ne peut pas dépasser ${remaining}€`, {
                duration: 3000,
                position: "top-right",
                style: {
                    background: "#ff4d4d",
            ...
            <div className="fixed inset-0 flex items-center justify-center z-50 p-4 sm:p-4 md:p-6">
                <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-2xl p-5 sm:p-6 md:p-10 border-1 border-gray-200">
                    ...
                    <div className="mb-4 sm:mb-6 md:mb-8">
                        <Title size="sm">
                            <span className="text-[#ff8200]">Contribuez</span>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 sm:gap-6 md:gap-8 items-stretch">
                        <div className="flex-shrink-0 w-full md:w-auto">
                            <div className="rounded-xl  w-full max-w-[200px] sm:max-w-xs md:w-56 mx-auto md:mx-0">
                        ...
                        <div className="flex-grow flex flex-col w-full min-w-0 justify-between ">
                            <div>
                            <h3 className="text-[#ff8200] font-bold text-lg sm:text-xl md:text-2xl mb-1 sm:mb-2 ClashDisplayBold truncate">
                                {organization.orgName}
                            </h3>
                                    ...
                                    <div className="flex items-center gap-1">
                                        <input
                                            ...
                                            onBlur={() => {
                                                ...
                                                else if (amount > remaining) setAmount(remaining);
                                            }}
                                        ...
                                        />
                                        <span className="text-[#0a548d] font-bold">€</span>
                                    </div>
                                    <button
                                        onClick={handleIncrement}
                                        disabled={amount >= remaining}
                                        className={`rounded w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center font-bold text-lg flex-shrink-0 transition-colors ${amount >= remaining
                                            ? "bg-gray-400 text-gray-600 cursor-not-allowed"
                                            : "bg-[#0a548d] text-white hover:bg-[#073d66]"
                                            }`}
                                        type="button"
                                    >
                                        <Plus size={16} />
                            ...
                            <div className="flex flex-col gap-6 justify-end items-center">
                                ...
                                <button
                                    onClick={()=>handleShareOptions(organization)}
                                    className="hover:bg-[#fffbf5] rounded-lg transition-colors flex items-center gap-2 mr-16"
                                    title="Partager le lien"
                                >
                                    <img src='/images/fleche.jpeg' className="w-9 h-9 flex-shrink-0" />
                                    <span className="text-xl font-semibold Archivo text-[#ff7a00] hidden sm:inline underline underline-offset-1">Partager</span>
                                </button>
                                </div>
                        </div>
